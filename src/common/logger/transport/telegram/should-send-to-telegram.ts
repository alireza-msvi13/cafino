...
export function isShouldSendToTelegram(info: any): boolean {
  ...
  const { level = 'info', statusCode, message = '', stack = '' } = info;
  ...
  const hasStatusAbove =
    !LoggerConfig.telegram.onlyStatusAbove ||
    statusCode >= LoggerConfig.telegram.onlyStatusAbove;

  const hasCriticalKeyword = LoggerConfig.telegram.criticalKeywords.some(
    (kw) => message.includes(kw) || stack.includes(kw),
  );
