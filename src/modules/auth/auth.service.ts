...
export class AuthService {
  ...
  async sendOtp(phone: string, response: Response): Promise<Response> {
  ...
  async verfiyOtp(
    ...
    try {
      ...
      const tokens = await this.createTokens(user.phone, user.id.toString());
      const rtHash = await bcrypt.hash(tokens.refreshToken, 10);
      await this.userService.saveRefreshToken(phone, rtHash);
      response.cookie('access-token', tokens.accessToken, AccessCookieConfig);
      response.cookie(
        'refresh-token',
        tokens.refreshToken,
        RefreshCookieConfig,
