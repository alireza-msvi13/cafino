...
export class CartService {
  ...
  async addToCart(cartDto: CartDto, userId: string): Promise<ServerResponse> {
  ...
  async deleteCart(userId: string): Promise<ServerResponse> {
  ...
  async getCart(userId: string): Promise<ServerResponse> {
    const {
      totalAmount,
      totalDiscount,
      paymentAmount,
      ...
      try {
        ...
        if (!item) {
          skippedItems.push({
            itemId,
            itemName: 'Unknown',
            reason: 'Item not exist or hidden.',
          });
          continue;
        }
        ...
        const isAvailable = await this.itemService.hasSufficientStock(
          itemId,
          totalCount,
        );
        if (!isAvailable) {
  ...
  async getUserCart(userId: string) {
    ...
    const cartItems = cart.map(({ item, count }) => {
      ...
      const itemDiscount = Math.round(
        item.discount > 0 ? itemTotalPrice * (item.discount / 100) : 0,
      );
    ...
    if (activeGeneralDiscount?.discount?.active) {
      ...
      if (discount?.limit && discount.limit > discount.usage) {
        let discountAmount =
          discount.percent > 0
            ? paymentAmount * (discount.percent / 100)
            : discount.amount || 0;
